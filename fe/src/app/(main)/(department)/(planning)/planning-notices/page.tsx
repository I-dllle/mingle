...
export default function PlanningNoticesPage() {
  ...
  console.log("Looking for path:", "/planning-notices");
  ...
  const handlePostClick = async (postId: number) => {
    ...
    try {
      setNavigating(true);
      const targetUrl = `/planning-notices/${postId}`;
  ...
  const loadPosts = async (page: number = 1) => {
    ...
    try {
      ...
      // 기획 부서 공지사항 메뉴 ID로 설정 (현재 메뉴 ID 사용)
