...
export default function ActivityPlanningPage() {
  ...
  const currentMenu = menus.find((menu) => menu.path === "/activity-planning");
  const boardName = currentMenu?.name || "활동기획";
  ...
  const handlePostClick = async (postId: number) => {
    ...
    try {
      setNavigating(true);
      const targetUrl = `/activity-planning/${postId}`;
  ...
  const loadPosts = async (page: number = 1) => {
    ...
    try {
      ...
      // 메뉴 ID를 13으로 하드코딩하여 부서별 게시글 조회
      const response = await postService.getPostsByMenu(deptId, 13);
      setPosts(response);
