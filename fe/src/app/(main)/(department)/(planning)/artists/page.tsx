...
export default function ArtistsPage() {
  ...
  const currentMenu = menus.find((menu) => menu.path === "/artists");
  const boardName = currentMenu?.name || "아티스트";
  ...
  const loadPosts = async (page: number = 1) => {
    ...
    try {
      ...
      // 메뉴 ID를 12로 하드코딩하여 부서별 게시글 조회
      const response = await postService.getPostsByMenu(deptId, 12);
      setPosts(response);
