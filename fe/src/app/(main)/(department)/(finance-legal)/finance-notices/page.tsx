...
export default function FinanceNoticesPage() {
  ...
  const currentMenu = menus.find((menu) => menu.path === "/finance-notices");
  ...
  console.log("Looking for path:", "/finance-notices");
  ...
  const handlePostClick = async (postId: number) => {
    ...
    try {
      setNavigating(true);
      const targetUrl = `/finance-notices/${postId}`;
  ...
  const loadPosts = async (page: number = 1) => {
    ...
    try {
      ...
      // 재정법무 부서 공지사항 메뉴 ID로 설정 (현재 메뉴 ID 사용)
