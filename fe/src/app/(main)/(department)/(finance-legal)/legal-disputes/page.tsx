...
export default function LegalDisputesPage() {
  ...
  const currentMenu = menus.find((menu) => menu.path === "/legal-disputes");
  const boardName = currentMenu?.name || "법률 분쟁 내역";

  // 디버깅용
  console.log("User Department:", userDepartment);
  console.log("Available menus:", menus);
  console.log("Current menu:", currentMenu);
  console.log("Looking for path:", "/legal-disputes");
  ...
  const handlePostClick = async (postId: number) => {
    ...
    try {
      setNavigating(true);
      const targetUrl = `/legal-disputes/${postId}`;
  ...
  const loadPosts = async (page: number = 1) => {
    ...
    try {
      ...
      // 메뉴 ID를 19로 하드코딩하여 부서별 게시글 조회
      const response = await postService.getPostsByMenu(deptId, 19);
      setPosts(response);
