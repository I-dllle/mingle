...
export default function CreativeNoticesPage() {
  ...
  console.log("Looking for path:", "/creative-notices");
  ...
  const handlePostClick = async (postId: number) => {
    ...
    try {
      setNavigating(true);
      const targetUrl = `/creative-notices/${postId}`;
  ...
  const loadPosts = async (page: number = 1) => {
    ...
    try {
      ...
      // 크리에이티브 부서 공지사항 메뉴 ID로 설정 (현재 메뉴 ID 사용)
