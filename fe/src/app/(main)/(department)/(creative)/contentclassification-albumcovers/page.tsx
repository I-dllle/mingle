...
export default function ContentclassificationAlbumcoversPage() {
  const { name: userDepartment } = useDepartment();
  const menus = departmentMenus[userDepartment] || departmentMenus.default;
  const currentMenu = menus.find(
    (menu) => menu.path === "/contentclassification-albumcovers"
  );
  const boardName = currentMenu?.name || "앨범커버";

  // 디버깅용
  console.log("User Department:", userDepartment);
  console.log("Available menus:", menus);
  console.log("Current menu:", currentMenu);
  console.log("Looking for path:", "/contentclassification-albumcovers");
  ...
  const handlePostClick = async (postId: number) => {
    ...
    try {
      setNavigating(true);
      const targetUrl = `/contentclassification-albumcovers/${postId}`;
  ...
  const loadPosts = async (page: number = 1) => {
    ...
    try {
      ...
      // 메뉴 ID를 10으로 하드코딩하여 부서별 게시글 조회
      const response = await postService.getPostsByMenu(deptId, 10);
      setPosts(response);
