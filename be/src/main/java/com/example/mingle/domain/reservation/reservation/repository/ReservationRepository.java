...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    boolean existsByRoomIdAndDateAndStartTimeLessThanAndEndTimeGreaterThan(
            Long roomId, LocalDate date, LocalTime newEnd, LocalTime newStart
    );
    ...
    @Query("SELECT COUNT(r) > 0 FROM Reservation r " +
            "WHERE r.room.id = :roomId AND r.date = :date AND " +
            "r.startTime < :newEnd AND r.endTime > :newStart AND " +
            "r.id != :reservationId")
    boolean existsOverlappingReservationExceptThis(
